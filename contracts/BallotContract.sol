...
contract BallotContract {
    ...
    mapping (uint256 => uint) private countResult;
    mapping (uint256 => HelperCounter) public helperCounters; //uint256 ProposalId
    mapping (address => string) public voterNames;
    mapping (uint256 => mapping ( uint => Vote)) private votes; // A proposal id is the key for the vote counter that maps a vote, this is private so in theory is not accessible.
    mapping (address => mapping ( uint256 => bool)) public voterRegister;
    enum State { Created, Voting, Ended }
    mapping (uint => VotingProposal) public proposals;

    //MODIFIERS
    modifier onlyOfficial(uint256 _id){
        VotingProposal memory _proposal = proposals[_id];
        require(_proposal.creatorAddress == msg.sender);
        _;
    }

    modifier inState(State _state ,  uint256 _id){
        VotingProposal memory _proposal = proposals[_id];
        require(_proposal.status == _state);
        _;
}
    modifier hasNotVoted (uint256 _id){
        VotingProposal memory _proposal = proposals[_id];
        require(bytes(voterNames[msg.sender]).length != 0
    ...
    function createProposal (string memory _officialName, string memory _description) public {
        proposalCounter++;
        proposals[proposalCounter] = VotingProposal(msg.sender, proposalCounter,_officialName,_description,block.timestamp,State.Created);
        helperCounters[proposalCounter] = HelperCounter(0,0,0);

    }
    function addVoter(uint256 _proposalId, address _voterAddress, string memory _voterName)
        public
