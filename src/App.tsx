...
const App: React.FunctionComponent<AppProps> = ( ) => {
  ...
  async function getBallotContract() {
    ...
    const ballotContract = new web3.eth.Contract(contractAbi, contractNetwork) as unknown as BallotContract;
  ...
  async function getProposals() {
    ...
    const proposals:IProposal[] = await Promise.all(
      Array.from({ length: web3.utils.toNumber(proposalCounter) }, (_, i) => i + 1)
        .map(async (i) => await contract.methods.proposals(i).call())
    );

    return setProposalList(proposalList.concat(proposals.sort((a, b) => web3.utils.toNumber(b.proposalId) - web3.utils.toNumber(a.proposalId))));
  }
