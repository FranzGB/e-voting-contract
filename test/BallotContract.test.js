...
contract("BallotContract", function (accounts) {
  ...
  describe("Functionality", () => {
    ...
    it("adds a voter to a proposal", async () => {
      const tx = await ballotContract.addVoter(proposalCounter, "John Doe");
      assert.equal(tx.receipt.status, true);
    ...
    it("creates a vote in the proposal", async () => {
      const exerciseVote = await ballotContract.doVote(proposalCounter, true, {
        from: accounts[0],
      });
    ...
    it("ends the voting on the proposal and returns the number of votes", async () => {
      ...
      const newProposal = await ballotContract.proposals(proposalCounter);
      const counters = await ballotContract.helperCounters(proposalCounter);
      assert.equal(tx.receipt.status, true);
      assert.equal(newProposal.status, BallotContract.State.Ended);
      assert.notEqual(counters.totalVotesInFavor.toNumber(), 5);
      assert.equal(counters.totalVotesInFavor.toNumber(), 1);
    });
  });
});
